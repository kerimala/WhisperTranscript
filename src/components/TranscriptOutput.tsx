...
import DiarizedPreview from './DiarizedPreview';
...
type ViewMode = 'preview' | 'json' | 'text' | 'diarized' | 'analysis';

export default function TranscriptOutput({ result, onReset }: TranscriptOutputProps) {
    const hasSpeakerSegments = result.segments.some((segment) => Boolean(segment.speaker));
    const [copied, setCopied] = useState<'json' | 'text' | 'diarized' | 'analysis' | null>(null);
    const [viewMode, setViewMode] = useState<ViewMode>(hasSpeakerSegments ? 'preview' : 'text');
    ...
    return (
        <div className="w-full space-y-6">
            ...
            <div className="flex gap-2 p-1 bg-slate-100 rounded-lg w-fit">
                {hasSpeakerSegments && (
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded-md transition-colors flex items-center gap-1.5 ${viewMode === 'preview' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-600 hover:text-slate-800'
                            }`}
                    >
            ...
            {/* Content */}
            <div className={viewMode === 'preview' ? 'relative' : 'relative bg-slate-900 rounded-xl overflow-hidden'}>
                {/* Action buttons */}
                ...
                {viewMode === 'preview' ? (
                    <DiarizedPreview segments={result.segments} />
                ) : (
